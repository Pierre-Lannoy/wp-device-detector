...
use PODeviceDetector\System\Plugin;
	...
	class Menus {
		...
		public static function finalize() {
			if ( ! self::$initialized ) {
				...
				add_submenu_page( 'perfopsone-dashboard', esc_html__( 'Control Center', 'device-detector' ), __( 'Control Center', 'device-detector' ), 'manage_options', 'perfopsone-dashboard', [ self::class, 'get_settings_page' ] );
			}
			...
			foreach ( self::$menus_positions as $menu ) {
				if ( ! self::$initialized ) {
					switch ( $menu ) {
						case 'analytics':
							add_submenu_page( 'perfopsone-dashboard', esc_html__( 'Analytics', 'device-detector' ), __( 'Analytics', 'device-detector' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_analytics_page' ] );
							break;
						case 'tools':
							add_submenu_page( 'perfopsone-dashboard', esc_html__( 'Tools', 'device-detector' ), __( 'Tools', 'device-detector' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_tools_page' ] );
							break;
						case 'insights':
							add_submenu_page( 'perfopsone-dashboard', esc_html__( 'Reports', 'device-detector' ), __( 'Reports', 'device-detector' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_insights_page' ] );
							break;
						case 'records':
							add_submenu_page( 'perfopsone-dashboard', esc_html__( 'Catalogues', 'device-detector' ), __( 'Catalogues', 'device-detector' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_records_page' ] );
							break;
						case 'consoles':
							add_submenu_page( 'perfopsone-dashboard', esc_html__( 'Consoles', 'device-detector' ), __( 'Consoles', 'device-detector' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_consoles_page' ] );
							break;
					}
				}
			...
			if ( array_key_exists( 'analytics', self::$menus ) ) {
				...
				foreach ( self::$menus['analytics'] as $item ) {
					...
					if ( $item['activated'] ) {
						...
						$i['text'] = esc_html__( 'This analytics feature is currently disabled. Click here to activate it.', 'device-detector' );
						$i['url']  = $item['remedy'];
					}
					$items[] = $i;
				}
		...
		public static function get_settings_page() {
			if ( array_key_exists( 'settings', self::$menus ) ) {
				...
				foreach ( self::$menus['settings'] as $item ) {
					...
					if ( $d->waiting_update() ) {
						$i['need_update'] = sprintf( esc_html__( 'Need to be updated to %s.', 'device-detector' ), $d->waiting_update() );
					} else {
						$i['need_update'] = '';
					}
					if ( $d->is_required_wp_ok() ) {
						$i['need_wp_update'] = '';
						$i['ok_wp_update']   = esc_html__( 'OK', 'device-detector' );
					} else {
						$i['need_wp_update'] = esc_html__( 'need update', 'device-detector' );
						$i['ok_wp_update']   = '';
					}
					if ( $d->is_required_php_ok() ) {
						$i['need_php_update'] = '';
						$i['ok_php_update']   = esc_html__( 'OK', 'device-detector' );
					} else {
						$i['need_php_update'] = esc_html__( 'need update', 'device-detector' );
						$i['ok_php_update']   = '';
					}
					$i['icon'] = call_user_func( $item['icon_callback'] );
					$i['slug'] = $item['plugin'];
					...
					if ( 0 < (int) $i['installs'] ) {
						$i['installs'] = sprintf( esc_html__( '%s+ installs.', 'device-detector' ), Conversion::number_shorten( (int) $i['installs'], 0 ) );
					} else {
						$i['installs'] = '';
					}
					if ( 0 < (int) $i['downloads'] ) {
						$i['downloads'] = sprintf( esc_html__( '%s downloads.', 'device-detector' ), Conversion::number_shorten( (int) $i['downloads'], 2 ) );
					} else {
						$i['downloads'] = '';
					}
					if ( 0 < (int) $i['reviews'] ) {
						$i['reviews'] = sprintf( esc_html__( '%s reviews.', 'device-detector' ), Conversion::number_shorten( (int) $i['reviews'], 0 ) );
						$i['rating']  = $i['rating'] / 20;
					} else {
						$i['reviews'] = esc_html__( 'No review yet.', 'device-detector' );
						$i['rating']  = 0.0;
					}
					$i['stars'] = '';
					if ( 0 < (int) $i['rating'] ) {
		...
		private static function display_as_controls( $items ) {
			...
			$disp       .= '   } );';
			$disp       .= '  </script>';
			$main_update = '<span class="poo-noneed-update">' . esc_html__( 'Up to date', 'device-detector' ) . '</span>';
			foreach ( $items as $item ) {
				if ( $item['need_update'] ) {
					$update      = '<a href=" ' .admin_url( 'update-core.php' ) . '"><span class="poo-need-update">' . esc_html__( 'Need update', 'device-detector' ) . '</span></a>';
					$main_update = $update;
				} else {
					$update = '<span class="poo-noneed-update">' . esc_html__( 'Up to date', 'device-detector' ) . '</span>';
				}
				if ( ! current_user_can( 'update_plugins' ) || ! wp_is_auto_update_enabled_for_type( 'plugin' ) || ( is_multisite() && ! is_network_admin() ) ) {
					$auto = '';
				} else {
					$auto  = '<div class="poo-switch" data-value="' . $item['slug'] . '">' . esc_html__( 'auto-update', 'device-detector' ) . ' &nbsp;';
				...
				$support  = '<a title="' . esc_html__( 'Get support', 'device-detector' ) . '" alt="' . esc_html__( 'Get support', 'device-detector' ) . '" class="poo-links-support" href="https://wordpress.org/support/plugin/' . $item['slug'] . '" target="_blank"><img style="width:16px;padding:0 4px;" src="' . \Feather\Icons::get_base64( 'message-circle', 'none', '#73879C', 3 ) . '" /></a>';
				$contrib  = '<a title="' . esc_html__( 'Contribute', 'device-detector' ) . '" alt="' . esc_html__( 'Contribute', 'device-detector' ) . '" class="poo-links-contrib" href="https://github.com/Pierre-Lannoy/wp-' . $item['slug'] . '" target="_blank"><img style="width:16px;padding:0 4px;" src="' . \Feather\Icons::get_base64( 'github', 'none', '#73879C', 3 ) . '" /></a>';
				$star     = '<a title="' . esc_html__( 'Make a review', 'device-detector' ) . '" alt="' . esc_html__( 'Make a review', 'device-detector' ) . '" class="poo-links-star" href="https://wordpress.org/support/plugin/' . $item['slug'] . '/reviews" target="_blank"><img style="width:16px;padding:0 4px;" src="' . \Feather\Icons::get_base64( 'star', 'none', '#73879C', 3 ) . '" /></a>';
				$settings = '<a class="poo-action" href="' . $item['url'] . '"/><img style="width:18px;padding:0 4px;" src="' . \Feather\Icons::get_base64( 'settings', 'none', '#73879C', 3 ) . '" />' . esc_html__( 'settings', 'device-detector' ) . '</a>';
				$about    = '<a class="poo-action" href="' . $item['url'] . '&tab=about"/><img style="width:18px;padding:0 4px;" src="' . \Feather\Icons::get_base64( 'info', 'none', '#73879C', 3 ) . '" />' . esc_html__( 'about', 'device-detector' ) . '</a>';
			...
			$site  = '<a class="poo-action" href="https://perfops.one" target="_blank"><img style="width:18px;padding:0 4px;vertical-align: bottom !important;" src="' . \Feather\Icons::get_base64( 'home', 'none', '#73879C', 3 ) . '" />' . esc_html__( 'all plugins', 'device-detector' ) . '</a>';
			$disp .= '<div class="poo-container">';
			$disp .= ' <div class="poo-actionable">';

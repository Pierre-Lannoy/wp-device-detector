...
class Analytics {
	...
	private function query_events() {
		...
		$result .= '<tr>';
		$result .= '<th>&nbsp;</th>';
		$result .= '<th>' . esc_html__( 'Timeframe', 'device-detector' ) . '</th>';
		$result .= '<th>' . esc_html__( 'Details', 'device-detector' ) . '</th>';
		$result .= '</tr>';
		$found   = false;
		foreach ( $data as $key => $row ) {
			...
			switch ( $row['status'] ) {
				...
				case 'cache_full':
					if ( array_key_exists( $key - 1, $data ) && 'cache_full' !== $data[ $key - 1 ]['status'] ) {
						...
						$name = esc_html__( 'Cache is full.', 'device-detector' );
						$op   = $row['status'];
					}
					break;
			}
			switch ( $row['status'] ) {
				case 'disabled':
					if ( array_key_exists( $key - 1, $data ) && 'disabled' !== $data[ $key - 1 ]['status'] ) {
						...
						$name = esc_html__( 'OPcache disabled.', 'device-detector' );
						$op   = $row['status'];
					}
					break;
				default:
					if ( array_key_exists( $key - 1, $data ) && 'disabled' === $data[ $key - 1 ]['status'] ) {
						$icon = '<img style="width:14px;vertical-align:text-bottom;" src="' . Feather\Icons::get_base64( 'power', 'none', '#73879C' ) . '" />';
						$name = esc_html__( 'OPcache enabled.', 'device-detector' );
						$op   = 'enabled';
					}
			}
			...
			if ( 0 < count( $conf ) && 'disabled' !== $op && 'enabled' !== $op && 'recycle_in_progress' !== $op ) {
				foreach ( $conf as $idx ) {
					...
					switch ( $idx ) {
						case 'mem':
							if ( 0 < $val ) {
								$str[] = sprintf( esc_html__( 'Total memory size increased by %s.', 'device-detector' ), Conversion::data_shorten( abs( $val ), 0, false, '&nbsp;' ) );
							} elseif ( 0 > $val ) {
								$str[] = sprintf( esc_html__( 'Total memory size decreased by %s.', 'device-detector' ), Conversion::data_shorten( abs( $val ), 0, false, '&nbsp;' ) );
							}
							$op = 'settings';
							break;
						case 'buf':
							if ( 0 < $val ) {
								$str[] = sprintf( esc_html__( 'Total buffer size increased by %s.', 'device-detector' ), Conversion::data_shorten( abs( $val ), 0, false, '&nbsp;' ) );
							} elseif ( 0 > $val ) {
								$str[] = sprintf( esc_html__( 'Total buffer size decreased by %s.', 'device-detector' ), Conversion::data_shorten( abs( $val ), 0, false, '&nbsp;' ) );
							}
							$op = 'settings';
							break;
						case 'key':
							if ( 0 < $val ) {
								$str[] = sprintf( esc_html__( 'Maximum keys slots increased by %s.', 'device-detector' ), Conversion::number_shorten( abs( $val ), 0, false, '' ) );
							} elseif ( 0 > $val ) {
								$str[] = sprintf( esc_html__( 'Maximum keys slots decreased by %s.', 'device-detector' ), Conversion::number_shorten( abs( $val ), 0, false, '' ) );
							}
							$op = 'settings';
							break;
					}
			...
			if ( 0 < count( $str ) ) {
				...
				$sname    = esc_html__( 'Settings changed.', 'device-detector' );
				$sdetails = implode( ' ', $str );
				$row_str  = '<tr>';
				$row_str .= '<td data-th="">' . $sicon . '&nbsp;&nbsp;<span class="podd-table-text">' . $sname . '</span></td>';
				$row_str .= '<td data-th="' . esc_html__( 'Timeframe', 'device-detector' ) . '">' . $time . '</td>';
				$row_str .= '<td data-th="' . esc_html__( 'Details', 'device-detector' ) . '">' . $sdetails . '</td>';
				$row_str .= '</tr>';
				$result  .= $row_str;
			}
			$str = [];
			switch ( $op ) {
				...
				case 'reset_warmup':
					if ( array_key_exists( $key - 1, $data ) ) {
						...
						if ( 0 < $val ) {
							$str[] = sprintf( esc_html__( 'Free memory size increased by %s.', 'device-detector' ), Conversion::data_shorten( abs( $val ), 0, false, '&nbsp;' ) );
						} elseif ( 0 > $val ) {
							$str[] = sprintf( esc_html__( 'Free memory size decreased by %s.', 'device-detector' ), Conversion::data_shorten( abs( $val ), 0, false, '&nbsp;' ) );
						}
						$val = ( $row['buf_total'] - $row['buf_used'] ) - ( $data[ $key - 1 ]['buf_total'] - $data[ $key - 1 ]['buf_used'] );
						if ( 0 < $val ) {
							$str[] = sprintf( esc_html__( 'Free buffer size increased by %s.', 'device-detector' ), Conversion::data_shorten( abs( $val ), 0, false, '&nbsp;' ) );
						} elseif ( 0 > $val ) {
							$str[] = sprintf( esc_html__( 'Free buffer size decreased by %s.', 'device-detector' ), Conversion::data_shorten( abs( $val ), 0, false, '&nbsp;' ) );
						}
						$val = ( $row['key_total'] - $row['key_used'] ) - ( $data[ $key - 1 ]['key_total'] - $data[ $key - 1 ]['key_used'] );
						if ( 0 < $val ) {
							$str[] = sprintf( esc_html__( 'Free keys slots increased by %s.', 'device-detector' ), Conversion::number_shorten( abs( $val ), 0, false, '' ) );
						} elseif ( 0 > $val ) {
							$str[] = sprintf( esc_html__( 'Free keys slots decreased by %s.', 'device-detector' ), Conversion::number_shorten( abs( $val ), 0, false, '' ) );
						}
					}
					$details = implode( ' ', $str );
					break;
				...
				case 'enabled':
					break;
				case 'cache_full':
					$details = sprintf( esc_html__( 'Current wasted memory: %s.', 'device-detector' ), Conversion::data_shorten( $row['mem_wasted'], 0, false, '&nbsp;' ) );
					break;
			}
			if ( '' === $details ) {
				$details = '-';
			...
			$row_str  = '<tr>';
			$row_str .= '<td data-th="">' . $icon . '&nbsp;&nbsp;<span class="podd-table-text">' . $name . '</span></td>';
			$row_str .= '<td data-th="' . esc_html__( 'Timeframe', 'device-detector' ) . '">' . $time . '</td>';
			$row_str .= '<td data-th="' . esc_html__( 'Details', 'device-detector' ) . '">' . $details . '</td>';
			$row_str .= '</tr>';
			$result  .= $row_str;
		}
		if ( ! $found ) {
			$row_str  = '<tr>';
			$row_str .= '<td data-th=""><em>' . esc_html__( 'No status events in the selected time range.', 'device-detector' ) . '</em></span></td>';
			$row_str .= '<td data-th="' . esc_html__( 'Timeframe', 'device-detector' ) . '">&nbsp;</td>';
			$row_str .= '<td data-th="' . esc_html__( 'Details', 'device-detector' ) . '">&nbsp;</td>';
			$row_str .= '</tr>';
			$result  .= $row_str;
		}
		$result .= '</table>';
		return [ 'podd-events' => $result ];
	}
		...
		if ( 0 !== count( $query ) ) {
			...
			$json_ratio        = wp_json_encode(
				[
					'series' => [
						[
							'name' => esc_html_x( 'Hit Ratio', 'Noun - Cache hit ratio.', 'device-detector' ),
							'data' => $series['ratio'],
						],
					],
				]
			...
			$json_availability        = wp_json_encode(
				[
					'series' => [
						[
							'name' => esc_html__( 'Availability', 'device-detector' ),
			...
			$json_hit         = wp_json_encode(
				[
					'series' => [
						[
							'name' => esc_html__( 'Hit Count', 'device-detector' ),
							'data' => $series['hit'],
						],
						[
							'name' => esc_html__( 'Miss Count', 'device-detector' ),
							'data' => $series['miss'],
						],
					],
				]
			...
			$json_scripts        = wp_json_encode(
				[
					'series' => [
						[
							'name' => esc_html__( 'Files Count', 'device-detector' ),
							'data' => $series['scripts'],
						],
					],
				]
			...
			$json_strings        = wp_json_encode(
				[
					'series' => [
						[
							'name' => esc_html__( 'Strings Count', 'device-detector' ),
							'data' => $series['strings'],
						],
					],
				]
			...
			$json_memory = wp_json_encode(
				[
					...
					'series' => [
						[
							'name' => esc_html__( 'Used Memory', 'device-detector' ),
							'data' => $series['memory'][0],
						],
						[
							'name' => esc_html__( 'Free Memory', 'device-detector' ),
							'data' => $series['memory'][1],
						],
						[
							'name' => esc_html__( 'Wasted Memory', 'device-detector' ),
			...
			$json_key = wp_json_encode(
				[
					...
					'series' => [
						[
							'name' => esc_html__( 'Used Key Slots', 'device-detector' ),
							'data' => $series['key'][0],
						],
						[
							'name' => esc_html__( 'Free Key Slots', 'device-detector' ),
							'data' => $series['key'][1],
						],
					],
				]
			...
			$json_buf = wp_json_encode(
				[
					...
					'series' => [
						[
							'name' => esc_html__( 'Used Buffer', 'device-detector' ),
							'data' => $series['buf'][0],
						],
						[
							'name' => esc_html__( 'Free Buffer', 'device-detector' ),
							'data' => $series['buf'][1],
						],
					],
				]
			...
			$result .= '<div class="podd-multichart-item active" id="podd-chart-ratio">';
			$result .= '</div>';
			$result .= '<script>';
			...
			$result .= ' };';
			$result .= ' new Chartist.Line("#podd-chart-ratio", ratio_data' . $uuid . ', ratio_option' . $uuid . ');';
			$result .= '});';
			$result .= '</script>';
			...
			$result .= '});';
			$result .= '</script>';
			$result .= '<div class="podd-multichart-item" id="podd-chart-hit">';
			$result .= '</div>';
			$result .= '<script>';
			$result .= 'jQuery(function ($) {';
			...
			$result .= ' };';
			$result .= ' new Chartist.Line("#podd-chart-hit", hit_data' . $uuid . ', hit_option' . $uuid . ');';
			$result .= '});';
			$result .= '</script>';
			$result .= '<div class="podd-multichart-item" id="podd-chart-string">';
			$result .= '</div>';
			$result .= '<script>';
			$result .= 'jQuery(function ($) {';
			...
			$result .= ' };';
			$result .= ' new Chartist.Line("#podd-chart-string", string_data' . $uuid . ', string_option' . $uuid . ');';
			$result .= '});';
			$result .= '</script>';
			$result .= '<div class="podd-multichart-item" id="podd-chart-file">';
			$result .= '</div>';
			$result .= '<script>';
			$result .= 'jQuery(function ($) {';
			...
			$result .= ' };';
			$result .= ' new Chartist.Line("#podd-chart-file", file_data' . $uuid . ', file_option' . $uuid . ');';
			$result .= '});';
			$result .= '</script>';
			$result .= '<div class="podd-multichart-item" id="podd-chart-memory">';
			$result .= '<style>';
			if ( 1 < $this->duration ) {
				$result .= '.podd-multichart-item .ct-bar {stroke-width: 20px !important;stroke-opacity: 0.8 !important;}';
			} else {
				$result .= '.podd-multichart-item .ct-bar {stroke-width: 3px !important;stroke-opacity: 0.8 !important;}';
			}
			$result .= '</style>';
			$result .= '</div>';
			$result .= '<script>';
			...
			$result .= '  axisY: {showGrid: true, labelInterpolationFnc: function (value) {return value.toString() + " ' . esc_html_x( 'MB', 'Abbreviation - Stands for "megabytes".', 'device-detector' ) . '";}},';
			$result .= ' };';
			$result .= ' new Chartist.Bar("#podd-chart-memory", memory_data' . $uuid . ', memory_option' . $uuid . ');';
			$result .= '});';
			$result .= '</script>';
			$result .= '<div class="podd-multichart-item" id="podd-chart-buffer">';
			$result .= '<style>';
			if ( 1 < $this->duration ) {
				$result .= '.podd-multichart-item .ct-bar {stroke-width: 20px !important;stroke-opacity: 0.8 !important;}';
			} else {
			...
			$result .= '  axisY: {showGrid: true, labelInterpolationFnc: function (value) {return value.toString() + " ' . esc_html_x( 'MB', 'Abbreviation - Stands for "megabytes".', 'device-detector' ) . '";}},';
			$result .= ' };';
			$result .= ' new Chartist.Bar("#podd-chart-buffer", buffer_data' . $uuid . ', buffer_option' . $uuid . ');';
			$result .= '});';
			$result .= '</script>';
			$result .= '<div class="podd-multichart-item" id="podd-chart-key">';
			$result .= '<style>';
			if ( 1 < $this->duration ) {
				$result .= '.podd-multichart-item .ct-bar {stroke-width: 20px !important;stroke-opacity: 0.8 !important;}';
			} else {
			...
			$result .= '  axisY: {showGrid: true, labelInterpolationFnc: function (value) {return value.toString() + " ' . esc_html_x( 'K', 'Abbreviation - Stands for "thousand".', 'device-detector' ) . '";}},';
			$result .= ' };';
			$result .= ' new Chartist.Bar("#podd-chart-key", key_data' . $uuid . ', key_option' . $uuid . ');';
			$result .= '});';
			$result .= '</script>';
	...
	private function query_kpi( $queried ) {
			...
			switch ( $queried ) {
				case 'ratio':
					if ( is_array( $data ) && array_key_exists( 'sum_hit', $data ) ) {
						$result[ 'kpi-bottom-' . $queried ] = '<span class="podd-kpi-large-bottom-text">' . sprintf( esc_html__( '%s hits', 'device-detector' ), Conversion::number_shorten( $data['sum_hit'], 2, false, '&nbsp;' ) ) . '</span>';
					}
					break;
				case 'memory':
					$result[ 'kpi-bottom-' . $queried ] = '<span class="podd-kpi-large-bottom-text">' . sprintf( esc_html__( 'total memory: %s', 'device-detector' ), Conversion::data_shorten( $base_value, 0, false, '&nbsp;' ) ) . '</span>';
					break;
				case 'buffer':
					$result[ 'kpi-bottom-' . $queried ] = '<span class="podd-kpi-large-bottom-text">' . sprintf( esc_html__( 'buffer size: %s', 'device-detector' ), Conversion::data_shorten( $base_value, 0, false, '&nbsp;' ) ) . '</span>';
					break;
				case 'key':
					$result[ 'kpi-bottom-' . $queried ] = '<span class="podd-kpi-large-bottom-text">' . sprintf( esc_html__( '%s keys (avg.)', 'device-detector' ), (int) round( $data_value, 0 ) ) . '</span>';
					break;
				case 'uptime':
					if ( 0.0 !== $base_value ) {
						$duration = implode( ', ', Date::get_age_array_from_seconds( $this->duration * DAY_IN_SECONDS * ( $data_value / $base_value ), true, true ) );
						if ( '' === $duration ) {
							$duration = esc_html__( 'no availability', 'device-detector' );
						} else {
							$duration = sprintf( esc_html__( 'available %s', 'device-detector' ), $duration );
						}
		...
		if ( 'script' === $queried ) {
			...
			if ( is_array( $data ) && array_key_exists( 'min_scripts', $data ) && array_key_exists( 'max_scripts', $data ) ) {
				...
				$result[ 'kpi-bottom-' . $queried ] = '<span class="podd-kpi-large-bottom-text">' . (int) round( $data['min_scripts'], 0 ) . '&nbsp;<img style="width:12px;vertical-align:middle;" src="' . Feather\Icons::get_base64( 'arrow-right', 'none', '#73879C' ) . '" />&nbsp;' . (int) round( $data['max_scripts'], 0 ) . '&nbsp;</span>';
			}
		}
		return $result;
	}
	...
	public function get_title_bar() {
		...
		$result .= '<span class="podd-title">' . esc_html__( 'OPcache Analytics', 'device-detector' ) . '</span>';
		$result .= '<span class="podd-subtitle">' . OPcache::name() . '</span>';
	...
	public function get_kpi_bar() {
		...
		$result .= '<div class="podd-kpi-large">' . $this->get_large_kpi( 'ratio' ) . '</div>';
		$result .= '<div class="podd-kpi-large">' . $this->get_large_kpi( 'memory' ) . '</div>';
		$result .= '<div class="podd-kpi-large">' . $this->get_large_kpi( 'script' ) . '</div>';
		$result .= '<div class="podd-kpi-large">' . $this->get_large_kpi( 'key' ) . '</div>';
		$result .= '<div class="podd-kpi-large">' . $this->get_large_kpi( 'buffer' ) . '</div>';
	...
	public function get_main_chart() {
		$help_ratio  = esc_html__( 'Hit ratio variation.', 'device-detector' );
		$help_hit    = esc_html__( 'Hit and miss distribution.', 'device-detector' );
		$help_memory = esc_html__( 'Memory distribution.', 'device-detector' );
		$help_file   = esc_html__( 'Files variation.', 'device-detector' );
		$help_key    = esc_html__( 'Keys distribution.', 'device-detector' );
		$help_string = esc_html__( 'Strings variation.', 'device-detector' );
		$help_buffer = esc_html__( 'Buffer distribution.', 'device-detector' );
		$help_uptime = esc_html__( 'Availability variation.', 'device-detector' );
		$detail      = '<span class="podd-chart-button not-ready left" id="podd-chart-button-ratio" data-position="left" data-tooltip="' . $help_ratio . '"><img style="width:12px;vertical-align:baseline;" src="' . Feather\Icons::get_base64( 'award', 'none', '#73879C' ) . '" /></span>';
		$detail     .= '&nbsp;&nbsp;&nbsp;<span class="podd-chart-button not-ready left" id="podd-chart-button-hit" data-position="left" data-tooltip="' . $help_hit . '"><img style="width:12px;vertical-align:baseline;" src="' . Feather\Icons::get_base64( 'hash', 'none', '#73879C' ) . '" /></span>';
		$detail     .= '&nbsp;&nbsp;&nbsp;<span class="podd-chart-button not-ready left" id="podd-chart-button-memory" data-position="left" data-tooltip="' . $help_memory . '"><img style="width:12px;vertical-align:baseline;" src="' . Feather\Icons::get_base64( 'cpu', 'none', '#73879C' ) . '" /></span>';
		$detail     .= '&nbsp;&nbsp;&nbsp;<span class="podd-chart-button not-ready left" id="podd-chart-button-file" data-position="left" data-tooltip="' . $help_file . '"><img style="width:12px;vertical-align:baseline;" src="' . Feather\Icons::get_base64( 'file-text', 'none', '#73879C' ) . '" /></span>';
		$detail     .= '&nbsp;&nbsp;&nbsp;<span class="podd-chart-button not-ready left" id="podd-chart-button-key" data-position="left" data-tooltip="' . $help_key . '"><img style="width:12px;vertical-align:baseline;" src="' . Feather\Icons::get_base64( 'key', 'none', '#73879C' ) . '" /></span>';
		$detail     .= '&nbsp;&nbsp;&nbsp;<span class="podd-chart-button not-ready left" id="podd-chart-button-string" data-position="left" data-tooltip="' . $help_string . '"><img style="width:12px;vertical-align:baseline;" src="' . Feather\Icons::get_base64( 'tag', 'none', '#73879C' ) . '" /></span>';
		$detail     .= '&nbsp;&nbsp;&nbsp;<span class="podd-chart-button not-ready left" id="podd-chart-button-buffer" data-position="left" data-tooltip="' . $help_buffer . '"><img style="width:12px;vertical-align:baseline;" src="' . Feather\Icons::get_base64( 'database', 'none', '#73879C' ) . '" /></span>';
	...
	public function get_events_list() {
		$result  = '<div class="podd-box podd-box-full-line">';
		$result .= '<div class="podd-module-title-bar"><span class="podd-module-title">' . esc_html__( 'Status Events', 'device-detector' ) . '</span></div>';
		$result .= '<div class="podd-module-content" id="podd-events">' . $this->get_graph_placeholder( 200 ) . '</div>';
		$result .= '</div>';
		$result .= $this->get_refresh_script(
			[
				'query'   => 'events',
	...
	private function get_large_kpi( $kpi ) {
		switch ( $kpi ) {
			case 'ratio':
				...
				$title = esc_html_x( 'Hit Ratio', 'Noun - Cache hit ratio.', 'device-detector' );
				$help  = esc_html__( 'The ratio between hit and total calls.', 'device-detector' );
				break;
			case 'memory':
				$icon  = Feather\Icons::get_base64( 'cpu', 'none', '#73879C' );
				$title = esc_html_x( 'Free Memory', 'Noun - Memory free of allocation.', 'device-detector' );
				$help  = esc_html__( 'Ratio of free available memory.', 'device-detector' );
				break;
			case 'script':
				$icon  = Feather\Icons::get_base64( 'file-text', 'none', '#73879C' );
				$title = esc_html_x( 'Cached Files', 'Noun - Number of already cached files.', 'device-detector' );
				$help  = esc_html__( 'Number of compiled and cached files.', 'device-detector' );
				break;
			case 'key':
				$icon  = Feather\Icons::get_base64( 'key', 'none', '#73879C' );
				$title = esc_html_x( 'Keys Saturation', 'Noun - Ratio of the allocated keys to the total available keys slots.', 'device-detector' );
				$help  = esc_html__( 'Ratio of the allocated keys to the total available keys slots.', 'device-detector' );
				break;
			case 'buffer':
				$icon  = Feather\Icons::get_base64( 'database', 'none', '#73879C' );
				$title = esc_html_x( 'Buffer Saturation', 'Noun - Ratio of the used buffer to the total buffer size.', 'device-detector' );
				$help  = esc_html__( 'Ratio of the used buffer to the total buffer size.', 'device-detector' );
				break;
			case 'uptime':
				$icon  = Feather\Icons::get_base64( 'activity', 'none', '#73879C' );
				$title = esc_html_x( 'Availability', 'Noun - Ratio of time when OPcache is not disabled.', 'device-detector' );
				$help  = esc_html__( 'Time ratio with an operational OPcache.', 'device-detector' );
				break;
		}
	...
	private function get_refresh_script( $args = [] ) {
		...
		if ( array_key_exists( 'query', $args ) && 'main-chart' === $args['query'] ) {
			...
			$result .= '$("#podd-chart-button-ratio").addClass("active");';
		}
		$result .= ' });';
		$result .= '});';
		$result .= '</script>';

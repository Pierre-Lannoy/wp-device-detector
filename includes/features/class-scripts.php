...
class Scripts extends \WP_List_Table {
	...
	public function get_columns() {
		$columns = [
			...
			'script'    => esc_html__( 'File', 'device-detector' ),
			'timestamp' => esc_html__( 'Timestamp', 'device-detector' ),
			'hit'       => esc_html__( 'Hits', 'device-detector' ),
			'memory'    => esc_html__( 'Memory size', 'device-detector' ),
			'used'      => esc_html__( 'Used', 'device-detector' ),
		];
		return $columns;
	}
	...
	public function get_bulk_actions() {
		return [
			'invalidate' => esc_html__( 'Invalidate', 'device-detector' ),
			'force'      => esc_html__( 'Force invalidate', 'device-detector' ),
			'recompile'  => esc_html__( 'Recompile', 'device-detector' ),
		];
	}
	...
	protected function display_tablenav( $which ) {
		if ( 'top' === $which ) {
			wp_nonce_field( 'bulk-podd-tools', '_wpnonce', false );
		}
	...
	public function extra_tablenav( $which ) {
		...
		if ( 'top' === $which || 'bottom' === $which ) {
			include PODD_ADMIN_DIR . 'partials/device-detector-admin-tools-lines.php';
		}
	}
	...
	public function get_line_number_select() {
		...
		foreach ( $_disp as $d ) {
			...
			$l['value'] = $d;
			// phpcs:ignore
			$l['text']     = sprintf( esc_html__( 'Display %d files per page', 'device-detector' ), $d );
	...
	public function warning() {
		...
		if ( function_exists( 'opcache_get_status' ) ) {
			...
			if ( ! (bool) $raw['opcache_enabled'] ) {
				$message = esc_html__( 'OPcache is not enabled on this site. There\'s nothing to see here.', 'device-detector' );
			}
			if ( (bool) $raw['restart_pending'] ) {
				$message = esc_html__( 'A full reset is currently pending. Displayed values may be inaccurate.', 'device-detector' );
			}
			if ( (bool) $raw['restart_in_progress'] ) {
				$message = esc_html__( 'A full reset is currently in progress. Displayed values may be inaccurate.', 'device-detector' );
			}
		} else {
			$message = esc_html__( 'OPcache is not enabled on this site. There\'s nothing to see here.', 'device-detector' );
		}
		if ( '' !== $message ) {
			// phpcs:ignore
			echo '<div id="podd-warning" class="notice notice-warning"><p><strong>' . $message . '</strong></p></div>';
		}
	}
	...
	public function process_args() {
		...
		foreach ( [ 'top', 'bottom' ] as $which ) {
			if ( wp_verify_nonce( $this->nonce, 'bulk-podd-tools' ) && array_key_exists( 'dolimit-' . $which, $_POST ) ) {
		...
		foreach ( [ 'top', 'bottom' ] as $which ) {
			if ( wp_verify_nonce( $this->nonce, 'bulk-podd-tools' ) && array_key_exists( 'dowarmup-' . $which, $_POST ) ) {
				$this->action = 'warmup';
			}
			if ( wp_verify_nonce( $this->nonce, 'bulk-podd-tools' ) && array_key_exists( 'doinvalidate-' . $which, $_POST ) ) {
				$this->action = 'reset';
			}
		}
		if ( '' === $this->action ) {
			$action = '-1';
			if ( '-1' === $action && wp_verify_nonce( $this->nonce, 'bulk-podd-tools' ) && array_key_exists( 'action', $_POST ) ) {
				$action = filter_input( INPUT_POST, 'action', FILTER_SANITIZE_STRING );
			}
			if ( '-1' === $action && wp_verify_nonce( $this->nonce, 'bulk-podd-tools' ) && array_key_exists( 'action2', $_POST ) ) {
				$action = filter_input( INPUT_POST, 'action2', FILTER_SANITIZE_STRING );
			}
			if ( '-1' !== $action && wp_verify_nonce( $this->nonce, 'bulk-podd-tools' ) && array_key_exists( 'bulk', $_POST ) ) {
		...
		switch ( $this->action ) {
			case 'warmup':
				if ( Environment::is_wordpress_multisite() ) {
					// phpcs:ignore
					$message = esc_html( sprintf( __( 'Network warm-up has been initiated. %d relevant files.', 'device-detector' ), OPcache::warmup( false ) ) );
				} else {
					// phpcs:ignore
					$message = esc_html( sprintf( __( 'Site warm-up has been initiated. %d relevant files.', 'device-detector' ), OPcache::warmup( false ) ) );
				}
				$code = 0;
				break;
			case 'reset':
				OPcache::reset( false );
				$message = esc_html__( 'Site invalidation has been initiated. It may take a few seconds to complete.', 'device-detector' );
				$code    = 0;
				break;
			case 'invalidate':
				// phpcs:ignore
				$message = esc_html( sprintf( __( 'Invalidation done: %d file(s).', 'device-detector' ), OPcache::invalidate( $this->bulk, false ) ) );
				$code    = 0;
				break;
			case 'force':
				// phpcs:ignore
				$message = esc_html( sprintf( __( 'Forced invalidation done: %d file(s).', 'device-detector' ), OPcache::invalidate( $this->bulk, true ) ) );
				$code    = 0;
				break;
			case 'recompile':
				// phpcs:ignore
				$message = esc_html( sprintf( __( 'Recompilation done: %d file(s).', 'device-detector' ), OPcache::recompile( $this->bulk, true ) ) );
				$code    = 0;
				break;
			default:
				return;

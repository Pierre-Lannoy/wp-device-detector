...
class Cache {
	 ...
	 */
	private static $pool_name = PODD_SLUG;
	...
	public static function init() {
		...
		add_action( 'shutdown', [ 'PODeviceDetector\System\Cache', 'log_debug' ], 10, 0 );
		add_filter( 'perfopsone_icache_introspection', [ 'PODeviceDetector\System\Cache', 'introspection' ] );
	}
	...
	public static function introspection( $endpoints ) {
		$endpoints[ PODD_SLUG ] = [ 'name' => PODD_PRODUCT_NAME, 'version' => PODD_VERSION, 'endpoint' => [ 'PODeviceDetector\System\Cache', 'get_analytics' ] ];
		return $endpoints;
	}
		...
		if ( self::$apcu_available ) {
			if ( function_exists( 'apcu_cache_info' ) && function_exists( 'apcu_delete' ) ) {
				try {
					...
					\DecaLog\Engine::eventsLogger( PODD_SLUG )->error( sprintf( 'Unable to query APCu status: %s.', $e->getMessage() ), [ 'code' => $e->getCode() ] );
					$result = 0;
				}
			}
		} elseif ( wp_using_ext_object_cache() ) {
	...
	public static function log_debug() {
		...
		if ( 0 !== (int) $analytics['hit']['count'] || 0 !== (int) $analytics['miss']['count'] ) {
			\DecaLog\Engine::eventsLogger( PODD_SLUG )->debug( $log );
		}
	}
